...
#SBATCH --output=report-bwa-pseudoref-Eu2
#SBATCH --mail-user=christina.wesse.osnabrueck@gmail.com
#SBATCH --mail-type=ALL
#SBATCH --job-name="pseudoref-Eu2-Trim-BWA"
...
samFolder="pseudoref-samresults-Eu2"
...
create_sam_file()
{
  files1=$(find $1 | grep "sample" | grep -v "T" | grep -v "G" | grep -v "C")
  files2="/rhome/cwesse/bigdata/CBP-RAD-CW/demux-jobs/Eu2/sample_P8-C-0-00-00.fastq.gz"		# This single sample slips through the "normal" find pattern because of its name differs from the usual nomenclature.It is added here manually to the rest
  files=$(echo -n ${files1}; echo  "" ${files2})
